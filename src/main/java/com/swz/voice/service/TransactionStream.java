...
public class TransactionStream extends Thread {
	...
	private static int currSend = 0, currSecond = 0, restTime = 0;
	/** 客户访问类表 **/
		...
		try {
			...
			if (jsonObject != null && jsonObject.getString("ret").equals("0")) {
				...
				if (ConfigDatas.getAlarmStyle().contains(code)
						|| AutoAlarmProc.hasVehicleId(alarm
								.getString("car_object_id"))
						|| alarmSignal != null) {
					sendFailInfo(alarm);
					// log.error("服务已转接人工台！");
					return;
				...
				// 获取数据库的联系人电话
				// 改用微信网页接口地址
				// String telePhone = DBManager.getUserPhone(carInfo
				// .getString("vehideid"));
				// "AlldayTel":"18666636693",
				String telePhone = JSONObject.fromObject(
						HttpServlet.sendGet(
								MYINFAddress,
								MYINF_USER.replace("VEHICLENUM",
										carInfo.getString("car_no")).replace(
										"OBJECTID",
				...
				if (m.matches()) {
					...
					alarmSignal.put("telphone", telePhone);
					alarmSignal.put("car_no", carInfo.getString("car_no"));
				} else { // 电话号码匹配失败，发送1到服务器
					throw new Exception("手机号码" + telePhone + "识别失败！");
				}
			} else {
				// log.info("目前没有数据需要处理");
			}
		} catch (Exception e) {
			log.error(GETPATH + " 服务器接口错误！" + e.getMessage());
			if (!alarm.isEmpty()) {
				// 程序特殊异常，发送1到服务器
				sendFailInfo(alarm);
			}
	...
	private String isTelephone(String telePhone) {
		if (TELPONENUM.contains(telePhone.substring(0, 6))) {
			// 本地电话
	...
	public static void sendUserPress(int currPort, boolean value) {
		...
		if (userInfo != null) {
			...
			try {
				HttpServlet.sendGet(PUTPATH, parameter);
			} catch (Exception e) {
				log.info("发送代码失败，请求人工手动发送以下内容：");
				log.error(PUTPATH + "?" + parameter);
			} finally {
				// 清除端口内容
				if (AutoAlarmProc.vehicleId.containsKey(currPort))
					AutoAlarmProc.vehicleId.remove(currPort);

				// 端口释放完成
				if (AutoAlarmProc.portBusy.containsKey(currPort))
					AutoAlarmProc.portBusy.remove(currPort);
			}
		} else {
			// log.info("vehicleId 没有当前端口" + currPort + "的数据,重复发送");
		}
	}
	...
	public synchronized static String getAlarmData(int currPort) {
		String result = null;
		if (alarmSignal != null) {
			result = alarmSignal.getString("telphone");
	...
	public static void setVisitInfos(List<VisitInfo> visitInfos) {
		TransactionStream.visitInfos = visitInfos;
	}
	...
	public void run() {
		while (!this.isThreadStop) {
			...
			// 正常数据交流
			if (currSend < (ConfigDatas.getPeopleSum() * 2)) {
				// 可以获取报警数据
				getHttpData();
			}
			// 报警数据已满
			...
			if (invailablePort > 0) {
				...
				if (alarmSignal == null
					...
					// 手动构造客服电话类型telphone,只有电话和，
					alarmSignal = new JSONObject();
					alarmSignal.put("telphone", AutoAlarmProc.telephoneList
							.get(0).split("/")[0]);
					alarmSignal.put("username", AutoAlarmProc.telephoneList
							.get(0).split("/")[1]);
				...
				// 无端口，不装箱
				if (invailablePort < 1 && alarmSignal != null) {
					sendFailInfo(alarmSignal);
					alarmSignal = null;
					// log.error("请检查设备，已经饱和了同志!一条电话线都没有，还接个什么劲警啊!?");
				}

				// 由于长时间没有可用端口导致服务器重新启动
				if ((restTime / 60) >= ConfigDatas.getTimeOut()) {
	...
	private static final String TELPONENUM = "1300580,1300581,1300582,1300583,1300584,1300585,1300586,1300587,1300588,1300589,"
			+ "1301058,"
